...
function GlobalTickerBox({ title, icon, items, isLoading }) {
  ...
  return (
    <div 
      className="col-span-1 md:col-span-4 rounded-2xl border p-4 md:p-5 flex flex-col h-[240px] sm:h-[260px] transition-all duration-500 hover:shadow-2xl hover:shadow-black/40"
      style={{ background: bg, borderColor: ring }}
    >
      <div className="flex justify-between items-center mb-3 md:mb-4">
        <div className="flex items-center gap-1.5 md:gap-2">
          <span className="material-symbols-outlined text-[18px] md:text-[22px]" style={{ color }}>{icon}</span>
          <span className="text-[10px] md:text-[12px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] text-white/90">{title}</span>
        </div>
        <span className="text-[9px] md:text-[10px] font-black px-1.5 md:px-2 py-0.5 rounded border uppercase tracking-widest shadow-sm" style={{ borderColor: ring, color }}>
...
export default function MarketOverview({ stocks, clusterSummary, macro, macroSentiment, supervisedValidation, panicMeter, totalStocks, generatedAt, indexName, isLoading }) {
  if (isLoading) {
    return <div className="col-span-1 lg:col-span-12 h-[400px] md:h-[540px] bg-surface-container rounded-xl animate-pulse" />;
  }
  ...
  return (
    <div className="col-span-1 lg:col-span-12 grid grid-cols-1 md:grid-cols-12 gap-4 md:gap-6 mb-4 md:mb-6">
      
      {/* 1. Macro Sentiment Gauge */}
      <div className="col-span-1 md:col-span-4 bg-surface-container border border-outline-variant rounded-xl p-4 md:p-6 flex flex-col h-[240px] sm:h-[260px] relative overflow-hidden">
        {/* Background Accent */}
        <div 
        ...
        <div className="flex justify-between items-start mb-3 md:mb-4 relative z-10">
          <h2 className="text-[10px] md:text-[11px] font-black text-white uppercase tracking-[0.15em] md:tracking-[0.2em]">Sentimen Makro</h2>
          <span className="material-symbols-outlined text-primary text-[18px] md:text-[20px] animate-pulse">public</span>
        </div>
        
        <div className="flex-1 flex flex-col justify-center items-center relative z-10">
          <span className="text-[9px] md:text-[10px] text-white/50 uppercase font-black tracking-widest mb-1">Regime Status</span>
          <span className={`text-2xl md:text-3xl font-black font-mono tracking-tighter ${isRiskOn ? 'text-semantic-bullish' : isBearish ? 'text-semantic-bearish' : 'text-amber-400'}`}>
            {macro?.macro_regime || 'NEUTRAL'}
          </span>
          
          <div className="w-full max-w-[160px] md:max-w-[200px] h-1.5 bg-white/10 rounded-full relative overflow-hidden mt-4 md:mt-6 mb-2">
            <div 
        ...
        <div className="flex flex-col items-center pt-3 md:pt-4 border-t border-white/10 relative z-10">
          <span className="text-[9px] md:text-[10px] text-white/50 uppercase font-black tracking-[0.15em] md:tracking-[0.2em] mb-1">Risk Adjusted Score</span>
          <span className="font-data-mono text-xl md:text-2xl font-black text-white leading-none">
      ...
      <div className="col-span-1 md:col-span-4 bg-surface-container border border-outline-variant rounded-xl p-3 md:p-4 flex flex-col justify-between h-[240px] sm:h-[260px] hover:bg-surface-variant/50 hover:-translate-y-0.5 transition-all duration-200 card-hover">
        ...
        <div className="flex-1 flex flex-col justify-center">
          <div className="flex items-baseline">
            <span className="text-3xl md:text-4xl font-bold text-on-surface">{totalStocks}</span>
            <span className="ml-2 text-[10px] md:text-xs text-on-surface-variant uppercase tracking-widest font-medium">Emiten</span>
          </div>
          <p className="text-[10px] md:text-[11px] text-on-surface-variant mt-2">Data historis 6 bulan terakhir dianalisis secara real-time.</p>
        </div>
        <div className="mt-2 md:mt-3 pt-2 border-t border-outline-variant flex justify-between font-data-mono text-[9px] md:text-[10px] text-on-surface-variant">
      ...
      <div className="col-span-1 md:col-span-4 bg-surface-container border border-outline-variant rounded-xl p-3 md:p-4 flex flex-col justify-between h-[240px] sm:h-[260px] hover:bg-surface-variant/50 hover:-translate-y-0.5 transition-all duration-200 card-hover">
        ...
        <div className="mt-2 md:mt-3 pt-2 border-t border-outline-variant flex justify-between font-data-mono text-[8px] md:text-[9px] text-on-surface-variant uppercase tracking-widest">
      ...
      {supervisedValidation && supervisedValidation.total_predictions > 0 && (
        <motion.div
          className="col-span-1 md:col-span-4 bg-surface-container border border-outline-variant rounded-xl p-4 md:p-5 flex flex-col gap-2 md:gap-3 h-[240px] sm:h-[260px]"
          initial={{ opacity: 0, y: 10 }}
          animate={{ opacity: 1, y: 0 }}
        >
          <div className="flex items-center gap-1.5 md:gap-2">
            <span className="material-symbols-outlined text-[16px] md:text-[18px] text-primary">model_training</span>
            <span className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.12em] md:tracking-[0.15em] text-white/90">XGBoost Validation</span>
          </div>
          <div className="grid grid-cols-2 gap-2 md:gap-3 flex-1">
            <div className="flex flex-col justify-center">
              <span className="text-[8px] md:text-[9px] text-white/40 font-black uppercase tracking-widest mb-1">30-Day Accuracy</span>
              <span className="text-2xl md:text-3xl font-black font-mono text-primary">
                {Math.round((supervisedValidation.accuracy ?? 0) * 100)}%
              </span>
            </div>
            <div className="flex flex-col justify-center">
              <span className="text-[8px] md:text-[9px] text-white/40 font-black uppercase tracking-widest mb-1">High Conviction Precision</span>
              <span className="text-2xl md:text-3xl font-black font-mono text-semantic-bullish">
                {Math.round((supervisedValidation.high_conviction_precision ?? 0) * 100)}%
              </span>
            </div>
            <div className="flex flex-col justify-center">
              <span className="text-[8px] md:text-[9px] text-white/40 font-black uppercase tracking-widest mb-1">Total Predictions</span>
              <span className="text-xl md:text-2xl font-black font-mono text-white/80">
                {supervisedValidation.total_predictions}
              </span>
            </div>
            <div className="flex flex-col justify-center">
              <span className="text-[8px] md:text-[9px] text-white/40 font-black uppercase tracking-widest mb-1">Avg Confidence</span>
          ...
          <div className="pt-2 md:pt-3 border-t border-white/5 text-[7px] md:text-[8px] text-white/20 font-black uppercase tracking-widest">
      ...
      {!isLoading && (
        ...
        >
          <div className="flex items-center gap-1.5 md:gap-2">
            <span className="material-symbols-outlined text-[16px] md:text-[18px] text-semantic-bullish">equalizer</span>
            <span className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.12em] md:tracking-[0.15em] text-white/90">Market Breadth</span>
          </div>
            ...
            return (
              <div className="flex-1 flex flex-col justify-center gap-3 md:gap-4">
                <div className="flex justify-between items-end">
                  <div className="flex flex-col">
                    <span className="text-2xl md:text-3xl font-black font-mono text-semantic-bullish">{up}</span>
                    <span className="text-[8px] md:text-[9px] text-white/40 font-black uppercase tracking-widest">Advances</span>
                  </div>
                  <div className="flex flex-col items-end">
                    <span className="text-2xl md:text-3xl font-black font-mono text-semantic-bearish">{down}</span>
                    <span className="text-[8px] md:text-[9px] text-white/40 font-black uppercase tracking-widest">Declines</span>
                  </div>
                </div>
                
                <div className="w-full h-2.5 md:h-3 bg-white/5 rounded-full overflow-hidden flex">
      ...
      {!isLoading && (() => {
        ...
        return (
          ...
          >
            <div className="flex justify-between items-center mb-3 md:mb-4">
              <div className="flex items-center gap-1.5 md:gap-2">
                <span className="material-symbols-outlined text-[18px] md:text-[22px]" style={{ color }}>leaderboard</span>
                <span className="text-[10px] md:text-[12px] font-black uppercase tracking-[0.15em] md:tracking-[0.2em] text-white/90">Sector Momentum</span>
              </div>
              <span className="text-[9px] md:text-[10px] font-black px-1.5 md:px-2 py-0.5 rounded border uppercase tracking-widest shadow-sm" style={{ borderColor: ring, color }}>
                {isUp ? 'Strong' : 'Weak'}
              </span>
            ...
            <div className="mt-2 md:mt-3 pt-2 md:pt-3 border-t border-white/5 flex justify-between items-center">
              <div className="flex flex-col">
                <span className="text-[7px] md:text-[8px] text-white/40 font-black uppercase tracking-widest">Avg Performance</span>
                <span className="text-xs md:text-sm font-black font-mono" style={{ color }}>
                  {avgIdxChange >= 0 ? '+' : ''}{avgIdxChange.toFixed(2)}%
                </span>
              </div>
              <div className="flex items-center gap-1.5 md:gap-2">
                 <span className="text-[8px] md:text-[9px] font-black text-white/30 uppercase tracking-widest">Calculated</span>
                 <div className={`w-1.5 md:w-2 h-1.5 md:h-2 rounded-full ${isUp ? 'bg-semantic-bullish shadow-[0_0_8px_#00FFB2]' : 'bg-semantic-bearish shadow-[0_0_8px_#ef4444]'} animate-pulse`} />
              </div>
            </div>
          </motion.div>
        );
